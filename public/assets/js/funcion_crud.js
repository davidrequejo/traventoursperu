...
function cargar_siguiente_codigo(url, input_selector, span_charge = null) {
    // Mostrar spinner si se proporciona span_charge
    if (span_charge) {
        $(span_charge).html('<i class="fas fa-spinner fa-pulse fa-lg text-danger"></i>');
    }

    $.get(url, function (e, status) {
        try {
            // Asumiendo que ApiResponse devuelve { status: true, data: 12345 }
            if (e.status == true) {
                $(input_selector).val(e.data);
                if (span_charge) $(span_charge).html('');
            } else {
                ver_errores(e); // tu función global de errores
            }
        } catch (err) {
...
/*  ══════════════════════════════════════════ - A L E R T A S   T O A T S F Y - ══════════════════════════════════════════ */

function toastify_message({ message = "Mensaje",  type = "info", duration = 3000, gravity = "top", position = "right", close = true } = {}) {
  let background = "linear-gradient(to right,var(--primary-color), rgb(215, 124, 247))";
